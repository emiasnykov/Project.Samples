...
namespace Gluwa.SDK_dotnet.Tests
{
    class Api
    {
        ...
        public static RestRequest SendRequest(Method method, string privateKey)
        {
            var restRequest = new RestRequest(method);
            restRequest.AddHeader("Content-Type", "application/json");
            restRequest.AddHeader("X-REQUEST-SIGNATURE", SharedMethods.GetXRequestSignature(privateKey));
        ...
        public static RestClient SetUrl(string BASE_URI, string endpoint)
        {
            ...
            {
                Authenticator = new RestSharp.Authenticators.HttpBasicAuthenticator(SharedMethods.API_KEY_TEST, SharedMethods.API_SECRET_TEST)
            };
        }
        ...
        public static RestClient SetUrlAndClient(string BASE_URI, string endpoint)
        {
            ...
            return new RestClient(url)
            {
                Authenticator = new RestSharp.Authenticators.HttpBasicAuthenticator(SharedMethods.API_KEY, SharedMethods.API_SECRET)
            };
        }
    }
}
