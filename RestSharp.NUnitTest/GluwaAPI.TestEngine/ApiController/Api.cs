...
namespace GluwaAPI.TestEngine.ApiController
{
    public class Api : GluwaTestApi
    {
        ...
        /// <returns></returns>
        public static string GetBearerToken(string environment, string userName = "qa.assertible", string pass = "t3st0rder", string rngNo = "1878")
        {
            string tokenBody;
            string authUrl;
            if (IsTestEnvironment(environment) && (environment != "Sandbox"))    // Test environment  
            {
                ...
                tokenBody = $"client_id=qa.assertible&client_secret=qa.assertible&grant_type=password&username={userName}&password={pass}&scope=openid profile email username GluwaApi";
                authUrl = "https://auth-test.gluwa.com/";
            }
            ...
            {
                // identification for GluwacoinSenderAddress
                tokenBody = $"client_id=qa.restsharp&client_secret=t3st0rder&grant_type=password&username={userName}&password={pass}&scope=openid profile email username GluwaApi";
                authUrl = "https://sandbox.auth.gluwa.com/";
            }
            else if (environment == "Staging")    // Staging environment
            {
                // identification for GluwacoinMainnetSenderAddress
                tokenBody = $"client_id=qa.assertible&client_secret=EaMpTroUBuRTheDpOwEt&grant_type=password&username={userName}&password={pass}&scope=openid profile email username GluwaApi";
                authUrl = "https://auth-staging.gluwa.com/";
            }
            else                                   // Production environment
            {
                // identification for GluwacoinSenderAddress
                tokenBody = $"client_id=qa.assertible&client_secret=EaMpTroUBuRTheDpOwEt&grant_type=password&username={userName}&password={pass}&scope=openid profile email username GluwaApi";
                authUrl = "https://auth.gluwa.com/";
            }
